...
export const matrixSlice = createSlice({
	...
	reducers: {
		...
		initMatrix: (state, action) => {
			...
			newState.blackTimer = action.payload[0]
			newState.whiteTimer = action.payload[0]
			newState.status = state.status
			newState.value = matrix
			return newState
		},
		...
		findStaleMate(state) {
			...
			let whitePositionCount = state.allFreeCells.reduce(
				(count, freeCells, index) =>
					JSON.stringify(blackFreeArr.concat(whiteFreeArr)) === JSON.stringify(freeCells) && index % 2 === 1 ? count + 1 : count,
				0,
			)
			let blackPositonCount = state.allFreeCells.reduce(
				(count, freeCells, index) =>
					JSON.stringify(blackFreeArr.concat(whiteFreeArr)) === JSON.stringify(freeCells) && index % 2 === 0 ? count + 1 : count,
				0,
			)
			...
			function isCheckMatePossible(array, kingCount) {
				let hasQueenRookPawn = array.some(
					cellProps => cellProps.fig.name === "Queen" || cellProps.fig.name === "Pawn" || cellProps.fig.name === "Rook",
				)
				let bishopCount = array.filter(cellProps => cellProps.fig.name === "Bishop")
				let areSameSidedBishops =
					bishopCount.length + kingCount === array.length &&
					(bishopCount.every(cellProps => (cellProps.x + cellProps.y) % 2 === 0) ||
						bishopCount.every(cellProps => (cellProps.x + cellProps.y) % 2 === 1))
				let isOneBishop = bishopCount.length === 1
				let isOneKnight = array.filter(cellProps => cellProps.fig.name === "Knight").length === 1
				return (
					!hasQueenRookPawn &&
					(areSameSidedBishops || (array.length === 3 && (isOneBishop || isOneKnight)) || array.length === kingCount)
				)
			}
