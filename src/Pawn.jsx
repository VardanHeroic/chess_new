        ...
        this.props.matrix.map((row,cellX) => {
            row.map((cell,cellY) => {
				props.checkRay.forEach(checkRayCell => {
					if( 
						( ( moveDirections.includes(cell) && ( props.status !== 'check' || checkRayCell.key === cell.key  ) && (!cell.fig || cell.fig.name === 'Step'   ) || ( cell.fig  &&   attackedCells.includes(cell) && ( props.status !== 'check' || props.checkInitator.key === cell.key   )   )  ) && 
						(cellX > blockXmin && cellX < blockXmax) && (cellY > blockYmin && cellY < blockYmax)
					)) {
						freeCells.push(cell)
					}	
				})
